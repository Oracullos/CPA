object frmCronogramaAlteracoes: TfrmCronogramaAlteracoes
  Left = 418
  Top = 188
  BorderIcons = [biSystemMenu, biHelp]
  BorderStyle = bsSingle
  Caption = 'Cronograma de Altera'#231#245'es'
  ClientHeight = 557
  ClientWidth = 729
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  OnKeyDown = FormKeyDown
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object btnSalvar: TSpeedButton
    Left = 39
    Top = 508
    Width = 97
    Height = 45
    Cursor = crHandPoint
    Caption = '&Salvar'
    Flat = True
    Glyph.Data = {
      
      B2B2B4B2B2B4B2B2B2B2B2C1BAB8A56A6A7E2C2D823334FF00FF}
    OnClick = btnSalvarClick
  end
  object btnCancelar: TSpeedButton
    Left = 140
    Top = 508
    Width = 97
    Height = 45
    Cursor = crHandPoint
    Caption = 'C&ancelar'
    Flat = True
    Glyph.Data = {
      
      0303}
    NumGlyphs = 2
    OnClick = btnCancelarClick
  end
  object Label6: TLabel
    Left = 118
    Top = 48
    Width = 83
    Height = 13
    Caption = 'C'#243'd. Chamado'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object PageControl1: TPageControl
    Left = -1
    Top = 0
    Width = 729
    Height = 502
    ActivePage = TabSheet1
    TabOrder = 0
    object TabSheet1: TTabSheet
      Caption = 'Dados Gerais'
      object Label1: TLabel
        Left = 178
        Top = 28
        Width = 83
        Height = 13
        Caption = 'C'#243'd. Chamado'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label2: TLabel
        Left = 134
        Top = 61
        Width = 70
        Height = 13
        Caption = 'C'#243'd. Cliente'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label3: TLabel
        Left = 52
        Top = 89
        Width = 152
        Height = 13
        Caption = 'C'#243'd. Categoria p/ Projetos'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label4: TLabel
        Left = 132
        Top = 144
        Width = 72
        Height = 13
        Caption = 'C'#243'd. M'#243'dulo'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object btnBuscaCategoria: TSpeedButton
        Left = 576
        Top = 83
        Width = 33
        Height = 25
        Cursor = crHandPoint
        Flat = True
        Glyph.Data = {
          
          565656565656565656565656565656080F222216565656565656}
        Layout = blGlyphBottom
        OnClick = btnBuscaCategoriaClick
      end
      object btnBuscamodulo: TSpeedButton
        Left = 576
        Top = 139
        Width = 33
        Height = 25
        Cursor = crHandPoint
        Flat = True
        Glyph.Data = {
          
          565656565656565656565656565656080F222216565656565656}
        Layout = blGlyphBottom
        OnClick = btnBuscamoduloClick
      end
      object Label11: TLabel
        Left = 53
        Top = 118
        Width = 151
        Height = 13
        Caption = 'C'#243'd. Usu'#225'rio Respons'#225'vel'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object edtCodChamado: TEdit
        Left = 263
        Top = 23
        Width = 65
        Height = 21
        TabStop = False
        Enabled = False
        ReadOnly = True
        TabOrder = 0
      end
      object edtCodCliente: TDBEdit
        Left = 207
        Top = 57
        Width = 49
        Height = 21
        DataField = 'cod_cliente'
        DataSource = dsCronograma
        Enabled = False
        TabOrder = 1
        OnExit = edtCodClienteExit
      end
      object edtCodCategoria: TDBEdit
        Left = 207
        Top = 85
        Width = 49
        Height = 21
        DataField = 'cod_categoria'
        DataSource = dsCronograma
        TabOrder = 3
        OnExit = edtCodCategoriaExit
        OnKeyDown = edtCodCategoriaKeyDown
      end
      object edtNomeCliente: TEdit
        Left = 263
        Top = 57
        Width = 306
        Height = 21
        Enabled = False
        TabOrder = 2
      end
      object edtNomeCategoria: TEdit
        Left = 263
        Top = 85
        Width = 306
        Height = 21
        Enabled = False
        TabOrder = 4
      end
      object edtNomemodulo: TEdit
        Left = 263
        Top = 141
        Width = 306
        Height = 21
        Enabled = False
        TabOrder = 8
      end
      object Panel1: TPanel
        Left = 0
        Top = 165
        Width = 721
        Height = 312
        BevelInner = bvLowered
        TabOrder = 9
        object Label5: TLabel
          Left = 119
          Top = 15
          Width = 85
          Height = 13
          Caption = 'Dt. Solicita'#231#227'o'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label8: TLabel
          Left = 104
          Top = 43
          Width = 100
          Height = 13
          Caption = 'Dt. Prev. Entrega'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label15: TLabel
          Left = 130
          Top = 68
          Width = 74
          Height = 13
          Caption = 'Tempo Previsto'
        end
        object Label16: TLabel
          Left = 268
          Top = 70
          Width = 26
          Height = 13
          Caption = 'horas'
        end
        object Label10: TLabel
          Left = 96
          Top = 92
          Width = 108
          Height = 13
          Caption = 'Nome do Programa'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label12: TLabel
          Left = 100
          Top = 124
          Width = 105
          Height = 13
          Caption = 'Altera'#231#227'o Prevista'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object edtData: TDateTimePicker
          Left = 209
          Top = 12
          Width = 241
          Height = 21
          Date = 41037.000000000000000000
          Time = 41037.000000000000000000
          DateFormat = dfLong
          Enabled = False
          TabOrder = 0
        end
        object edtDataEntrega: TDateTimePicker
          Left = 209
          Top = 38
          Width = 241
          Height = 21
          Date = 41037.000000000000000000
          Time = 41037.000000000000000000
          DateFormat = dfLong
          TabOrder = 1
        end
        object edtTempo: TEdit
          Left = 209
          Top = 64
          Width = 57
          Height = 21
          TabOrder = 2
          Text = '0,00'
        end
        object edtDescPrograma: TDBEdit
          Left = 209
          Top = 88
          Width = 432
          Height = 21
          CharCase = ecUpperCase
          DataField = 'nome_programa'
          DataSource = dsCronograma
          TabOrder = 3
        end
        object mmoAltPrevista: TDBMemo
          Left = 208
          Top = 116
          Width = 433
          Height = 54
          DataField = 'desc_alteracao_prevista'
          DataSource = dsCronograma
          ScrollBars = ssVertical
          TabOrder = 4
          OnKeyPress = mmoAltPrevistaKeyPress
        end
        object cbxFinalizado: TDBCheckBox
          Left = 208
          Top = 176
          Width = 145
          Height = 17
          Caption = 'Altera'#231#227'o Finalizada?'
          DataField = 'finalizado'
          DataSource = dsCronograma
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
          TabOrder = 5
          ValueChecked = 'Sim'
          ValueUnchecked = 'N'#227'o'
          OnClick = cbxFinalizadoClick
        end
        object pnlAltFinalizacao: TPanel
          Left = 72
          Top = 193
          Width = 609
          Height = 114
          BevelInner = bvLowered
          TabOrder = 6
          object Label17: TLabel
            Left = 52
            Top = 12
            Width = 82
            Height = 13
            Caption = 'Data  Finaliza'#231#227'o'
          end
          object Label13: TLabel
            Left = 230
            Top = 12
            Width = 42
            Height = 13
            Caption = 'Hr. Final.'
          end
          object Label14: TLabel
            Left = 376
            Top = 12
            Width = 62
            Height = 13
            Caption = 'Tempo gasto'
          end
          object Label18: TLabel
            Left = 501
            Top = 12
            Width = 26
            Height = 13
            Caption = 'horas'
          end
          object Label9: TLabel
            Left = 40
            Top = 43
            Width = 93
            Height = 13
            Caption = 'Altera'#231#227'o Efetivada'
          end
          object edtDtFinal: TMaskEdit
            Left = 137
            Top = 8
            Width = 72
            Height = 21
            EditMask = '!99/99/9999;1; '
            MaxLength = 10
            TabOrder = 0
            Text = '  /  /    '
            OnExit = edtDtFinalExit
          end
          object edtHrFinal: TMaskEdit
            Left = 276
            Top = 8
            Width = 55
            Height = 21
            EditMask = '!90:00;1; '
            MaxLength = 5
            TabOrder = 1
            Text = '  :  '
          end
          object edtTmpGasto: TEdit
            Left = 442
            Top = 8
            Width = 57
            Height = 21
            TabOrder = 2
            Text = '0,00'
          end
          object mmoAltEfetivada: TMemo
            Left = 138
            Top = 40
            Width = 426
            Height = 67
            ScrollBars = ssVertical
            TabOrder = 3
            OnKeyPress = mmoAltEfetivadaKeyPress
          end
        end
      end
      object edtUserResp: TDBEdit
        Left = 208
        Top = 114
        Width = 49
        Height = 21
        DataField = 'cod_usr_responsavel'
        DataSource = dsCronograma
        Enabled = False
        TabOrder = 5
        OnExit = edtUserRespExit
      end
      object edtNomeUserResp: TEdit
        Left = 263
        Top = 114
        Width = 306
        Height = 21
        Enabled = False
        TabOrder = 6
      end
      object edtCodModulo: TDBEdit
        Left = 208
        Top = 141
        Width = 49
        Height = 21
        DataField = 'cod_modulo'
        DataSource = dsCronograma
        TabOrder = 7
        OnExit = edtCodModuloExit
        OnKeyDown = edtCodModuloKeyDown
      end
    end
  end
  object dsCronograma: TDataSource
    DataSet = dmPrincipal.cdsCronogramaAlt
    Left = 12
    Top = 120
  end
  object IdEnvioCli: TIdSMTP
    SASLMechanisms = <>
    Left = 556
    Top = 24
  end
  object IdMsg: TIdMessage
    AttachmentEncoding = 'MIME'
    BccList = <>
    CCList = <>
    Encoding = meMIME
    FromList = <
      item
      end>
    Recipients = <>
    ReplyTo = <>
    ConvertPreamble = True
    Left = 588
    Top = 24
  end
  object qrytempo_total: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQLConnection = dmPrincipal.conpg
    Left = 659
    Top = 80
  end
end
