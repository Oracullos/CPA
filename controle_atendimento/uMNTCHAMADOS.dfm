object frmCadChamados: TfrmCadChamados
  Left = 472
  Top = 156
  BorderIcons = [biSystemMenu, biHelp]
  BorderStyle = bsSingle
  Caption = 'Manuten'#231#227'o de Chamados'
  ClientHeight = 592
  ClientWidth = 731
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnKeyDown = FormKeyDown
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object btnSalvar: TSpeedButton
    Left = 39
    Top = 547
    Width = 97
    Height = 42
    Cursor = crHandPoint
    Caption = '&Salvar'
    Flat = True
    Glyph.Data = {
      
      B2B2B4B2B2B4B2B2B2B2B2C1BAB8A56A6A7E2C2D823334FF00FF}
    OnClick = btnSalvarClick
  end
  object btnCancelar: TSpeedButton
    Left = 140
    Top = 547
    Width = 97
    Height = 42
    Cursor = crHandPoint
    Caption = 'C&ancelar'
    Flat = True
    Glyph.Data = {
      
      0303}
    NumGlyphs = 2
    OnClick = btnCancelarClick
  end
  object pnlGeral: TPanel
    Left = 0
    Top = 0
    Width = 729
    Height = 545
    TabOrder = 0
    object PageControl1: TPageControl
      Left = 0
      Top = -3
      Width = 729
      Height = 545
      ActivePage = TabSheet1
      TabOrder = 0
      object TabSheet1: TTabSheet
        Caption = 'Dados Gerais'
        object Label1: TLabel
          Left = 118
          Top = 10
          Width = 83
          Height = 13
          Caption = 'C'#243'd. Chamado'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label2: TLabel
          Left = 131
          Top = 60
          Width = 70
          Height = 13
          Caption = 'C'#243'd. Cliente'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label3: TLabel
          Left = 116
          Top = 209
          Width = 85
          Height = 13
          Caption = 'C'#243'd. Categoria'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label4: TLabel
          Left = 134
          Top = 262
          Width = 67
          Height = 13
          Caption = 'C'#243'd. Status'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object btnBuscaCliente: TSpeedButton
          Left = 576
          Top = 48
          Width = 33
          Height = 25
          Cursor = crHandPoint
          Flat = True
          Glyph.Data = {
            
            565656565656565656565656565656080F222216565656565656}
          Layout = blGlyphBottom
          OnClick = btnBuscaClienteClick
        end
        object btnBuscaCategoria: TSpeedButton
          Left = 576
          Top = 199
          Width = 33
          Height = 25
          Cursor = crHandPoint
          Flat = True
          Glyph.Data = {
            
            565656565656565656565656565656080F222216565656565656}
          Layout = blGlyphBottom
          OnClick = btnBuscaCategoriaClick
        end
        object btnBuscaStatus: TSpeedButton
          Left = 576
          Top = 252
          Width = 33
          Height = 25
          Cursor = crHandPoint
          Flat = True
          Glyph.Data = {
            
            565656565656565656565656565656080F222216565656565656}
          Layout = blGlyphBottom
          OnClick = btnBuscaStatusClick
        end
        object Label11: TLabel
          Left = 50
          Top = 237
          Width = 151
          Height = 13
          Caption = 'C'#243'd. Usu'#225'rio Respons'#225'vel'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object btnBuscaUsuario: TSpeedButton
          Left = 576
          Top = 226
          Width = 33
          Height = 25
          Flat = True
          Glyph.Data = {
            
            565656565656565656565656565656080F222216565656565656}
          Layout = blGlyphBottom
          OnClick = btnBuscaUsuarioClick
        end
        object Label12: TLabel
          Left = 68
          Top = 34
          Width = 133
          Height = 13
          Caption = 'Usu'#225'rio Abriu Chamado'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label13: TLabel
          Left = 66
          Top = 84
          Width = 135
          Height = 13
          Caption = 'Solicitante do Chamado'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label20: TLabel
          Left = 137
          Top = 156
          Width = 64
          Height = 13
          Caption = 'Solicita'#231#227'o'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label21: TLabel
          Left = 108
          Top = 107
          Width = 93
          Height = 13
          Caption = 'E-mail do solicitante'
        end
        object edtBuscaEmail: TSpeedButton
          Left = 576
          Top = 101
          Width = 33
          Height = 25
          Cursor = crHandPoint
          Flat = True
          Glyph.Data = {
            
            565656565656565656565656565656080F222216565656565656}
          Layout = blGlyphBottom
          Visible = False
          OnClick = edtBuscaEmailClick
        end
        object btnBuscaCliUsuario: TSpeedButton
          Left = 576
          Top = 75
          Width = 33
          Height = 25
          Cursor = crHandPoint
          Flat = True
          Glyph.Data = {
            
            565656565656565656565656565656080F222216565656565656}
          Layout = blGlyphBottom
          OnClick = btnBuscaCliUsuarioClick
        end
        object Label19: TLabel
          Left = 135
          Top = 131
          Width = 66
          Height = 13
          Caption = 'Outros e-mails'
        end
        object edtCodChamado: TEdit
          Left = 207
          Top = 2
          Width = 49
          Height = 21
          TabStop = False
          ReadOnly = True
          TabOrder = 0
        end
        object edtCodCliente: TDBEdit
          Left = 207
          Top = 52
          Width = 49
          Height = 21
          DataField = 'cod_cliente'
          DataSource = dsChamado
          TabOrder = 4
          OnExit = edtCodClienteExit
          OnKeyDown = edtCodClienteKeyDown
        end
        object edtCodCategoria: TDBEdit
          Left = 207
          Top = 201
          Width = 49
          Height = 21
          DataField = 'cod_categoria'
          DataSource = dsChamado
          TabOrder = 11
          OnExit = edtCodCategoriaExit
          OnKeyDown = edtCodCategoriaKeyDown
        end
        object edtNomeCliente: TEdit
          Left = 263
          Top = 52
          Width = 306
          Height = 21
          Enabled = False
          TabOrder = 5
        end
        object edtNomeCategoria: TEdit
          Left = 263
          Top = 201
          Width = 306
          Height = 21
          Enabled = False
          TabOrder = 12
        end
        object edtNomeStatus: TEdit
          Left = 263
          Top = 254
          Width = 306
          Height = 21
          Enabled = False
          TabOrder = 16
        end
        object Panel2: TPanel
          Left = 0
          Top = 280
          Width = 721
          Height = 239
          BevelInner = bvLowered
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 17
          object Label5: TLabel
            Left = 141
            Top = 18
            Width = 28
            Height = 13
            Caption = 'Data'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
          end
          object Label6: TLabel
            Left = 529
            Top = 17
            Width = 28
            Height = 13
            Caption = 'Hora'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
          end
          object Label7: TLabel
            Left = 58
            Top = 135
            Width = 111
            Height = 13
            Caption = 'N'#237'vel de Prioridade'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
          end
          object Label8: TLabel
            Left = 79
            Top = 49
            Width = 90
            Height = 13
            Caption = 'Prazo do Chamado'
          end
          object Label9: TLabel
            Left = 58
            Top = 191
            Width = 111
            Height = 13
            Caption = 'Descri'#231#227'o do Chamado'
          end
          object Label10: TLabel
            Left = 74
            Top = 165
            Width = 95
            Height = 13
            Caption = 'Caminho do Arquivo'
          end
          object Label14: TLabel
            Left = 360
            Top = 135
            Width = 96
            Height = 13
            Caption = 'Origem Chamado'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
          end
          object Label15: TLabel
            Left = 105
            Top = 108
            Width = 64
            Height = 13
            Caption = 'Tempo Gasto'
          end
          object Label16: TLabel
            Left = 228
            Top = 108
            Width = 26
            Height = 13
            Caption = 'horas'
          end
          object Label17: TLabel
            Left = 87
            Top = 80
            Width = 82
            Height = 13
            Caption = 'Data  Finaliza'#231#227'o'
          end
          object Label18: TLabel
            Left = 310
            Top = 79
            Width = 79
            Height = 13
            Caption = 'Hora Finaliza'#231#227'o'
          end
          object Label22: TLabel
            Left = 371
            Top = 107
            Width = 15
            Height = 13
            Caption = 'Km'
          end
          object edtData: TDateTimePicker
            Left = 176
            Top = 10
            Width = 241
            Height = 21
            Date = 41037.000000000000000000
            Time = 41037.000000000000000000
            DateFormat = dfLong
            TabOrder = 0
            OnChange = edtDataChange
          end
          object cbxPrioridade: TComboBox
            Left = 176
            Top = 127
            Width = 121
            Height = 21
            CharCase = ecUpperCase
            ItemHeight = 13
            TabOrder = 8
            Items.Strings = (
              '1 - BAIXA'
              '2 - M'#201'DIA'
              '3 - ALTA')
          end
          object edtDataPrazo: TDateTimePicker
            Left = 176
            Top = 41
            Width = 241
            Height = 21
            Date = 41037.000000000000000000
            Time = 41037.000000000000000000
            DateFormat = dfLong
            TabOrder = 2
          end
          object mmoDescChamado: TDBMemo
            Left = 176
            Top = 190
            Width = 433
            Height = 49
            DataField = 'descricao_chamado'
            DataSource = dsChamado
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            ParentFont = False
            ScrollBars = ssVertical
            TabOrder = 12
            OnKeyPress = mmoDescChamadoKeyPress
          end
          object edtCaminhoArq: TDBEdit
            Left = 176
            Top = 157
            Width = 369
            Height = 21
            DataField = 'caminho_arq'
            DataSource = dsChamado
            TabOrder = 10
          end
          object edthora: TMaskEdit
            Left = 561
            Top = 10
            Width = 48
            Height = 21
            EditMask = '!90:00;1; '
            MaxLength = 5
            TabOrder = 1
            Text = '  :  '
          end
          object cbxOrigem: TDBComboBox
            Left = 463
            Top = 127
            Width = 145
            Height = 21
            CharCase = ecUpperCase
            DataField = 'origem_chamado'
            DataSource = dsChamado
            DropDownCount = 6
            ItemHeight = 13
            Items.Strings = (
              'TELEFONE'
              'E-MAIL'
              'SKYPE'
              'INTERNO'
              'EXTERNO')
            TabOrder = 9
          end
          object edtTempo: TEdit
            Left = 176
            Top = 100
            Width = 49
            Height = 21
            TabOrder = 7
            Text = '0'
          end
          object edtDtFinal: TMaskEdit
            Left = 176
            Top = 72
            Width = 65
            Height = 21
            EditMask = '!99/99/9999;1; '
            MaxLength = 10
            TabOrder = 5
            Text = '  /  /    '
            OnExit = edtDtFinalExit
          end
          object edtHoraFinal: TMaskEdit
            Left = 392
            Top = 71
            Width = 45
            Height = 21
            EditMask = '!90:00;1; '
            MaxLength = 5
            TabOrder = 6
            Text = '  :  '
            OnExit = edtHoraFinalExit
          end
          object chxCompromisso: TDBCheckBox
            Left = 500
            Top = 48
            Width = 113
            Height = 17
            Caption = 'Gera Compromisso'
            DataField = 'gera_compromisso'
            DataSource = dsChamado
            TabOrder = 3
            ValueChecked = 'Sim'
            ValueUnchecked = 'N'#227'o'
          end
          object btnAbrir: TBitBtn
            Left = 550
            Top = 155
            Width = 58
            Height = 25
            Caption = 'Abrir'
            TabOrder = 11
            TabStop = False
            OnClick = btnAbrirClick
            Glyph.Data = {
              
              00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
          end
          object chxCronograma: TDBCheckBox
            Left = 464
            Top = 74
            Width = 147
            Height = 17
            Caption = 'Gera Cronog. de Altera'#231#227'o'
            DataField = 'gera_cronograma_alt'
            DataSource = dsChamado
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            ParentFont = False
            TabOrder = 4
            ValueChecked = 'Sim'
            ValueUnchecked = 'N'#227'o'
          end
          object edtKm: TDBEdit
            Left = 392
            Top = 101
            Width = 68
            Height = 21
            TabStop = False
            DataField = 'km'
            DataSource = dsChamado
            TabOrder = 13
          end
        end
        object chxChamadoAutomatico: TDBCheckBox
          Left = 304
          Top = 2
          Width = 145
          Height = 17
          Caption = 'Chamado autom'#225'tico'
          DataField = 'chamado_automatico'
          DataSource = dsChamado
          ReadOnly = True
          TabOrder = 1
          ValueChecked = 'Sim'
          ValueUnchecked = 'N'#227'o'
        end
        object edtUserResp: TDBEdit
          Left = 208
          Top = 229
          Width = 49
          Height = 21
          DataField = 'cod_usuario_resp'
          DataSource = dsChamado
          TabOrder = 13
          OnExit = edtUserRespExit
          OnKeyDown = edtUserRespKeyDown
        end
        object edtNomeUserResp: TEdit
          Left = 263
          Top = 229
          Width = 306
          Height = 21
          Enabled = False
          TabOrder = 14
        end
        object edtCodUsuarioAbriu: TDBEdit
          Left = 207
          Top = 27
          Width = 49
          Height = 21
          DataField = 'cod_usuario_abriu'
          DataSource = dsChamado
          Enabled = False
          TabOrder = 2
        end
        object edtNomeUsuarioAbriu: TDBEdit
          Left = 263
          Top = 27
          Width = 306
          Height = 21
          DataField = 'nome_usuario_abriu'
          DataSource = dsChamado
          Enabled = False
          TabOrder = 3
        end
        object edtCodStatus: TDBEdit
          Left = 208
          Top = 254
          Width = 49
          Height = 21
          DataField = 'cod_status'
          DataSource = dsChamado
          TabOrder = 15
          OnExit = edtCodStatusExit
          OnKeyDown = edtCodStatusKeyDown
        end
        object edtsolicChamado: TDBEdit
          Left = 264
          Top = 78
          Width = 304
          Height = 21
          CharCase = ecUpperCase
          DataField = 'funcionario_emp'
          DataSource = dsChamado
          Enabled = False
          TabOrder = 7
        end
        object mmoSolicitacao: TDBMemo
          Left = 207
          Top = 151
          Width = 361
          Height = 46
          Hint = 
            'A descri'#231#227'o da solicita'#231#227'o ser'#225' inclu'#237'da no e-mail enviado ao cl' +
            'iente '#13#10'ap'#243's a finaliza'#231#227'o do atendimento.'
          DataField = 'solicitacao'
          DataSource = dsChamado
          ParentShowHint = False
          ScrollBars = ssVertical
          ShowHint = True
          TabOrder = 10
        end
        object edtemail_solicitante: TDBEdit
          Left = 207
          Top = 103
          Width = 362
          Height = 21
          Hint = 
            'Informe o e-mail do solicitante, para que o mesmo receba as noti' +
            'fica'#231#245'es'#13#10'de abertura e finaliza'#231#227'o de atendimentos.'
          DataField = 'email_solicitante'
          DataSource = dsChamado
          ParentShowHint = False
          ReadOnly = True
          ShowHint = True
          TabOrder = 8
          OnKeyDown = edtemail_solicitanteKeyDown
        end
        object edtUsuarioCli: TDBEdit
          Left = 207
          Top = 78
          Width = 49
          Height = 21
          DataField = 'cod_usuario_cli'
          DataSource = dsChamado
          TabOrder = 6
          OnExit = edtUsuarioCliExit
          OnKeyDown = edtUsuarioCliKeyDown
        end
        object edtOutrosEmail: TDBEdit
          Left = 207
          Top = 127
          Width = 362
          Height = 21
          Hint = 
            'Outros e-mails do solicitante, para que o mesmo receba as notifi' +
            'ca'#231#245'es'#13#10'de abertura e finaliza'#231#227'o de atendimentos.'
          DataField = 'email_solicitante_outro'
          DataSource = dsChamado
          ParentShowHint = False
          ShowHint = True
          TabOrder = 9
          OnKeyDown = edtemail_solicitanteKeyDown
        end
      end
    end
  end
  object dsChamado: TDataSource
    DataSet = dmPrincipal.cdsChamados
    Left = 12
    Top = 208
  end
  object IdEnvio: TIdSMTP
    SASLMechanisms = <>
    Left = 612
    Top = 48
  end
  object IdMsgTrocaResp: TIdMessage
    AttachmentEncoding = 'MIME'
    BccList = <>
    CCList = <>
    Encoding = meMIME
    FromList = <
      item
      end>
    Recipients = <>
    ReplyTo = <>
    ConvertPreamble = True
    Left = 644
    Top = 48
  end
  object IdEnvioCli: TIdSMTP
    SASLMechanisms = <>
    Left = 660
    Top = 136
  end
  object IdMsg: TIdMessage
    AttachmentEncoding = 'MIME'
    BccList = <>
    CCList = <>
    Encoding = meMIME
    FromList = <
      item
      end>
    Recipients = <>
    ReplyTo = <>
    ConvertPreamble = True
    Left = 692
    Top = 136
  end
  object OpenDialog: TOpenDialog
    FilterIndex = 0
    Options = [ofHideReadOnly, ofEnableSizing, ofDontAddToRecent]
    Left = 620
    Top = 439
  end
end
