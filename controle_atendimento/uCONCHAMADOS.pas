...
begin
     ...
     begin
          ...
          if dmPrincipal.qryStatusencerra_chamado.AsString = 'S' then
               ShowMessage('Impossível realizar movimentações. O chamado encontra-se encerrado !!')
               ...
               ShowMessage('Impossível realizar movimentações.'+#13+'Usuário logado difere do usuário responsável pelo chamado !!'+#13+
               'O chamado está atribuído para '+dmPrincipal.cdsChamadosnome_usuario_resp.AsString+', somente este usuário pode movimentar o chamado!');
...
begin
     ...
     //begin
          ...
          //begin
               ...
               begin
                    ShowMessage('Registro está sendo alterado pelo usuário: '+dmPrincipal.cdsBloqueadoslogin_bloqueou.AsString);
                    Abort;
               end
               else
               begin
          ...
          //end
          //else
          //ShowMessage('O chamado não pode ser alterado, pois encontra-se encerrado!!');
     //end;
               ...
               begin
                    ...
                    begin
                         ...
                         begin
                              ...
                              ShowMessage('O chamado encontra-se ENCERRADO. Impossível efetuar a exclusão!!');
                    end;
               end;
          end;
     end;
...
begin
     ...
     begin
          ...
          //valida campo de solicitação do chamado
          if (dmPrincipal.cdsChamadossolicitacao.AsString = '') then
          begin
               ShowMessage('A solicitação do chamado encontra-se vazia.'+#10+
                           'Verifique antes de prosseguir com a movimentação!' );
               Abort;
          end;
          ...
          begin
               ShowMessage('Impossível realizar movimentações.'+#13+'O status do chamado encontra-se ''EM COMPROMISSO'', verifique a aba de Compromissos do Chamado!');
               Abort;
          end;

          //valida a existência de cronograma de alterações
          if (dmPrincipal.cdsChamadosgera_cronograma_alt.AsString = 'S') then
          begin
               ShowMessage('Impossível realizar movimentações.'+#13+'Existe um cronograma de alterações definido para este chamado!');
               Abort;
          end;
          ...
          begin
               ...
               begin
                    ...
                    begin
                         ShowMessage('Chamado já foi movimentado por outro usuário !');
               ...
               begin
                    sReabrir := '';
                    if MessageDlg('O chamado encontra-se encerrado. Deseja reabrí-lo?',mtConfirmation,[mbYes, mbNo], 0) = mrYes then
                    begin
                         sReabrir := 'S';
                         dmPrincipal.qryMovChamado.Close;
                         dmPrincipal.qryMovChamado.SQL.Text := 'select * from movimento_chamado where '+
               ...
               //valida usuario responsável pelo chamado
               else if (dmPrincipal.cdsChamadoscod_usuario_resp.AsInteger <> cod_usuario_login) or (dmPrincipal.cdsChamadoscod_usuario_resp.AsString <> '') then
               begin
                    ShowMessage('Impossível realizar movimentações.'+#13+'Usuário logado difere do usuário responsável pelo chamado !!'+#13+
                              'O chamado está atribuído para '+dmPrincipal.cdsChamadosnome_usuario_resp.AsString+', somente este usuário pode movimentá-lo!');
               ...
               begin
                    ...
                    begin
                         ...
                         begin
                              ...
                              //  Busca de E-MAIL por usuários e DOMINIO do cliente
...
begin
     ...
     begin
          ...
          begin
               ShowMessage('Impossível realizar movimentações.'+#13+'Usuário logado difere do usuário responsável pelo compromisso !!'+#13+
                              'O Compromisso está atribuído para '+dmPrincipal.cdsChamadosnome_usuario_resp.AsString+', somente este usuário pode movimentá-lo!');
     ...
     begin
          ...
          begin
               ...
               begin
                    ...
                    ShowMessage('Impossível realizar movimentações.'+#13+'O cronograma de alteração encontra-se finalizado!');
          end
          else
          begin
               ShowMessage('Impossível realizar movimentações.'+#13+'Usuário logado difere do usuário responsável pela alteração !!'+#13+
                              'A alteração está atribuído para '+dmPrincipal.cdsChamadosnome_usuario_resp.AsString+', somente este usuário pode movimentá-lo!');
...
begin
     ...
     begin
          if (MessageDlg('Deseja reenviar o histórico das movimentações do chamado?',
                         mtConfirmation, [mbYes, mbNo], 0) = mrYes) then
          begin
               Screen.Cursor := crHourGlass;
               sprotocolo    := '';
...
begin
          ...
          begin
               ...
               begin
                    ...
                    begin
                         ...
                         if dmPrincipal.qryCategoria.Locate('cod_categoria',dmPrincipal.cdsChamadoscod_categoria.AsInteger,[]) then
                         begin
                              if (MessageDlg('Deseja reenviar a finalização do chamado?',
                                   mtConfirmation, [mbYes, mbNo], 0) = mrYes) then
                              begin
                                   if dmPrincipal.qryCategoriaenvia_email.AsString = 'S' then
                                   begin
                                   ...
                                   begin
                                        ShowMessage('Impossível efetuar o reenvio.'+#10+'A categoria do chamado está marcada para não enviar e-mail!');
                                        Abort;
                                   end;
                              end;
                         end;
     ...
     begin
          ShowMessage('Usuário logado difere do usuário responsável pelo chamado.');
     end;
end;
...
begin
     ...
     begin
          ...
          begin
               if (MessageDlg('Deseja reenviar a notificação de cobrança para o responsável?',
                              mtConfirmation, [mbYes, mbNo], 0) = mrYes) then
               begin
                    ...
                    try
                         ...
                         IdMsg.Body.Text := '[Oracullos - Mensagem Automática]'+#13+#13+
                                            ...
                                            'Tempo para cobrança (em horas): '+dmPrincipal.cdsChamadostempo_cobranca.AsString+#13+
                                            'Informamos a finalização do chamado Nº: '+sprotocolo+dmPrincipal.cdsChamadoscod_chamado.AsString+'.'+#13+
                                            'Data e Hora de Abertura: '+dmPrincipal.cdsChamadosdata_chamado.AsString+' - '+dmPrincipal.cdsChamadoshora_chamado.AsString+ #13+
                                            'Solicitante: '+dmPrincipal.cdsChamadosfuncionario_emp.AsString+#13+
                                            'Solicitação: '+dmPrincipal.cdsChamadossolicitacao.AsString+#13+
               ...
               ShowMessage('Não foi encontrado tempo de cobrança para o chamado selecionado!');
     end
     else
     begin
          ShowMessage('Usuário logado difere do usuário responsável pelo chamado.');
