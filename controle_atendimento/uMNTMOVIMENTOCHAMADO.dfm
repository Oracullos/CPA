object frmCadMovimentoChamado: TfrmCadMovimentoChamado
  Left = 539
  Top = 195
  ActiveControl = edtCodUsuario
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Manuten'#231#227'o Movimento do Chamado'
  ClientHeight = 518
  ClientWidth = 657
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnKeyDown = FormKeyDown
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object btnSalvar: TSpeedButton
    Left = 24
    Top = 464
    Width = 97
    Height = 49
    Cursor = crHandPoint
    Caption = '&Salvar'
    Flat = True
    Glyph.Data = {
      
      B2B2B4B2B2B4B2B2B2B2B2C1BAB8A56A6A7E2C2D823334FF00FF}
    OnClick = btnSalvarClick
  end
  object btnCancelar: TSpeedButton
    Left = 128
    Top = 464
    Width = 97
    Height = 49
    Cursor = crHandPoint
    Caption = 'C&ancelar'
    Flat = True
    Glyph.Data = {
      
      0303}
    NumGlyphs = 2
    OnClick = btnCancelarClick
  end
  object PageControl1: TPageControl
    Left = 0
    Top = 0
    Width = 657
    Height = 457
    ActivePage = TabSheet1
    TabOrder = 0
    object TabSheet1: TTabSheet
      Caption = 'Dados Gerais'
      object Label1: TLabel
        Left = 109
        Top = 16
        Width = 92
        Height = 13
        Caption = 'C'#243'd. Movimento'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label2: TLabel
        Left = 118
        Top = 40
        Width = 83
        Height = 13
        Caption = 'C'#243'd. Chamado'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label3: TLabel
        Left = 50
        Top = 89
        Width = 151
        Height = 13
        Caption = 'C'#243'd. Usu'#225'rio Respons'#225'vel'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label4: TLabel
        Left = 134
        Top = 115
        Width = 67
        Height = 13
        Caption = 'C'#243'd. Status'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object btnBuscaUsuario: TSpeedButton
        Left = 543
        Top = 77
        Width = 30
        Height = 25
        Cursor = crHandPoint
        Flat = True
        Glyph.Data = {
          
          565656565656565656565656565656080F222216565656565656}
        Layout = blGlyphBottom
        OnClick = btnBuscaUsuarioClick
      end
      object edtBuscaStatus: TSpeedButton
        Left = 543
        Top = 104
        Width = 30
        Height = 25
        Cursor = crHandPoint
        Flat = True
        Glyph.Data = {
          
          565656565656565656565656565656080F222216565656565656}
        OnClick = edtBuscaStatusClick
      end
      object Label10: TLabel
        Left = 62
        Top = 63
        Width = 139
        Height = 13
        Caption = 'C'#243'd. Usu'#225'rio Movimento'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object btnBuscaUsuarioMov: TSpeedButton
        Left = 544
        Top = 50
        Width = 30
        Height = 25
        Flat = True
        Glyph.Data = {
          
          565656565656565656565656565656080F222216565656565656}
        Layout = blGlyphBottom
        Visible = False
        OnClick = btnBuscaUsuarioMovClick
      end
      object Panel1: TPanel
        Left = 0
        Top = 132
        Width = 649
        Height = 305
        BevelInner = bvLowered
        TabOrder = 6
        object Label5: TLabel
          Left = 87
          Top = 29
          Width = 112
          Height = 13
          Caption = 'Dt. Inic. Movimento'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label6: TLabel
          Left = 88
          Top = 87
          Width = 111
          Height = 13
          Caption = 'N'#237'vel de Prioridade'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label7: TLabel
          Left = 77
          Top = 115
          Width = 124
          Height = 13
          Caption = 'Tempo gasto na Mov.'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label8: TLabel
          Left = 90
          Top = 176
          Width = 108
          Height = 13
          Caption = 'Intera'#231#227'o/Observa'#231#227'o'
        end
        object Label9: TLabel
          Left = 263
          Top = 115
          Width = 32
          Height = 13
          Caption = 'horas'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label11: TLabel
          Left = 93
          Top = 58
          Width = 106
          Height = 13
          Caption = 'Dt. Fim Movimento'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label12: TLabel
          Left = 462
          Top = 29
          Width = 57
          Height = 13
          Caption = 'Hora Inic.'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label13: TLabel
          Left = 468
          Top = 58
          Width = 51
          Height = 13
          Caption = 'Hora Fim'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object edtData: TDateTimePicker
          Left = 208
          Top = 21
          Width = 241
          Height = 21
          Date = 41037.000000000000000000
          Time = 41037.000000000000000000
          DateFormat = dfLong
          TabOrder = 0
        end
        object cbxPrioridade: TComboBox
          Left = 208
          Top = 79
          Width = 145
          Height = 21
          ItemHeight = 13
          TabOrder = 4
          Items.Strings = (
            '1 - BAIXA'
            '2 - M'#201'DIA'
            '3 - ALTA')
        end
        object edtTempo: TEdit
          Left = 208
          Top = 107
          Width = 49
          Height = 21
          TabOrder = 7
          Text = '0'
        end
        object mmoObservaco: TMemo
          Left = 88
          Top = 192
          Width = 489
          Height = 97
          ScrollBars = ssVertical
          TabOrder = 9
          OnKeyPress = mmoObservacoKeyPress
        end
        object edtData_fim: TDateTimePicker
          Left = 208
          Top = 50
          Width = 241
          Height = 21
          Date = 41096.000000000000000000
          Time = 41096.000000000000000000
          DateFormat = dfLong
          TabOrder = 2
        end
        object edtHoraIni: TMaskEdit
          Left = 522
          Top = 21
          Width = 47
          Height = 21
          EditMask = '!90:00;1; '
          MaxLength = 5
          TabOrder = 1
          Text = '  :  '
        end
        object edtHoraFim: TMaskEdit
          Left = 522
          Top = 50
          Width = 47
          Height = 21
          EditMask = '!90:00;1; '
          MaxLength = 5
          TabOrder = 3
          Text = '  :  '
          OnExit = edtHoraFimExit
        end
        object chxGeraCompromisso: TCheckBox
          Left = 459
          Top = 83
          Width = 113
          Height = 17
          Caption = 'Gera Compromisso'
          TabOrder = 5
        end
        object chxCronograma: TCheckBox
          Left = 421
          Top = 110
          Width = 150
          Height = 17
          Caption = 'Gera Cronog. de Altera'#231#227'o '
          TabOrder = 6
        end
        object chkEnviaMov: TCheckBox
          Left = 208
          Top = 147
          Width = 241
          Height = 17
          Caption = 'Enviar e-mail de movimenta'#231#227'o'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          ParentFont = False
          TabOrder = 8
        end
      end
      object edtCodMovimento: TEdit
        Left = 208
        Top = 8
        Width = 49
        Height = 21
        TabStop = False
        Enabled = False
        TabOrder = 0
      end
      object edtCodChamado: TDBEdit
        Left = 208
        Top = 32
        Width = 49
        Height = 21
        DataField = 'cod_chamado'
        DataSource = dsCodChamado
        Enabled = False
        TabOrder = 1
      end
      object edtNomeUsuario: TEdit
        Left = 264
        Top = 81
        Width = 273
        Height = 21
        Enabled = False
        TabOrder = 5
      end
      object edtNomeStatus: TEdit
        Left = 264
        Top = 107
        Width = 273
        Height = 21
        Enabled = False
        TabOrder = 7
      end
      object edtCodUsuario: TEdit
        Left = 208
        Top = 81
        Width = 49
        Height = 21
        TabOrder = 3
        OnExit = edtCodUsuarioExit
        OnKeyDown = edtCodUsuarioKeyDown
      end
      object edtCodStatus: TEdit
        Left = 208
        Top = 107
        Width = 49
        Height = 21
        TabOrder = 4
        OnExit = edtCodStatusExit
        OnKeyDown = edtCodStatusKeyDown
      end
      object edtUsuarioMov: TEdit
        Left = 264
        Top = 56
        Width = 273
        Height = 21
        Enabled = False
        TabOrder = 8
      end
      object edtCodUserMov: TEdit
        Left = 208
        Top = 56
        Width = 49
        Height = 21
        Enabled = False
        TabOrder = 2
        OnExit = edtCodUserMovExit
      end
      object edtDataChamado: TDBEdit
        Left = 264
        Top = 32
        Width = 89
        Height = 21
        DataField = 'data_chamado'
        DataSource = dsCodChamado
        Enabled = False
        TabOrder = 9
      end
    end
  end
  object dsMovChamado: TDataSource
    DataSet = dmPrincipal.cdsMovChamado
    Left = 36
    Top = 168
  end
  object dsCodChamado: TDataSource
    DataSet = dmPrincipal.cdsChamados
    Left = 36
    Top = 208
  end
  object qryAdd: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQLConnection = dmPrincipal.conpg
    Left = 612
    Top = 48
  end
  object qrytempo_total: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQLConnection = dmPrincipal.conpg
    Left = 620
    Top = 104
  end
  object IdEnvio: TIdSMTP
    SASLMechanisms = <>
    Left = 588
    Top = 180
  end
  object IdMsgTrocaResp: TIdMessage
    AttachmentEncoding = 'MIME'
    BccList = <>
    CCList = <>
    Encoding = meMIME
    FromList = <
      item
      end>
    Recipients = <>
    ReplyTo = <>
    ConvertPreamble = True
    Left = 620
    Top = 180
  end
  object IdEnvioCli: TIdSMTP
    SASLMechanisms = <>
    Left = 588
    Top = 212
  end
  object IdMsg: TIdMessage
    AttachmentEncoding = 'MIME'
    BccList = <>
    CCList = <>
    Encoding = meMIME
    FromList = <
      item
      end>
    Recipients = <>
    ReplyTo = <>
    ConvertPreamble = True
    Left = 620
    Top = 212
  end
end
